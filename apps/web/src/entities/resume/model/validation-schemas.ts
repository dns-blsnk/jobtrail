...
function urlField(label: string, required = false) {
  ...
  return required ? base.required(`${label} is required`) : base.nullable().optional();
}
...
function dateField(required = false) {
  ...
  return required ? base.required('Date is required') : base.nullable().optional();
}
...
const experienceItemSchema = Yup.object({
  ...
  endDate: Yup.string().when('present', {
    ...
    then: (s) =>
      s
        .test('date-min', 'Enter a valid date (e.g. Jan 2023)', (v) => !v || v.trim().length >= 4)
